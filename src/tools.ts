...
const get_balance: Tool = {
  name: "get_balance",
  description:
    "Get the available account credit balance for the authenticated Porkbun account. Returns the balance in cents (integer) and a human-readable display string (e.g. `$12.34`). Call it before offering to buy anything: API purchases are paid from this prepaid balance and never from a card on file, so this number decides whether a registration, renewal or transfer can go through at all. Top-up is a website action (https://porkbun.com/account/credit); auto top-up, which refills from a saved payment method, is at https://porkbun.com/account/api.",
  inputSchema: {},
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: true },
  handler: async (config) => {

...
| `/mcp/no-purchases` | 83 | + `register_domain`, `renew_domain`, `transfer_domain`, `buy_closeout`, `create_hosting`; the secret-returning `get_ssl_bundle`, `create_wp_credentials`, `create_webhook`, `rotate_webhook_secret`; and `update_transfer_auth_code` (takes a transfer credential). Webhook `secret` fields are redacted in the rest | both directory listings. ChatGPT: OpenAI allows commerce "only for physical goods", no links to checkout pages, no auth secrets in tool output. Claude: Anthropic's policy bars software that "transfers money ... or executes financial transactions on behalf of users" |
